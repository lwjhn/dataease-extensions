...
@Component()
public class OscarDsProvider extends DefaultJdbcProvider {
    @Resource
    ...
    public Connection getConnection(DatasourceRequest datasourceRequest) throws Exception {
        OscarConfig oscarConfig = new Gson().fromJson(datasourceRequest.getDatasource().getConfiguration(), OscarConfig.class);

        String defaultDriver = oscarConfig.getDriver();
        String customDriver = oscarConfig.getCustomDriver();

        String url = oscarConfig.getJdbc();
        Properties props = new Properties();
        DeDriver deDriver = null;
        if (StringUtils.isNotEmpty(oscarConfig.getAuthMethod()) && oscarConfig.getAuthMethod().equalsIgnoreCase("kerberos")) {
            ...
            loginUserFromKeytab.invoke(null, oscarConfig.getUsername(), "/opt/dataease/conf/" + oscarConfig.getPassword());
        } else {
            if (StringUtils.isNotBlank(oscarConfig.getUsername())) {
                props.setProperty("user", oscarConfig.getUsername());
                if (StringUtils.isNotBlank(oscarConfig.getPassword())) {
                    props.setProperty("password", oscarConfig.getPassword());
                }
            }
        }
    ...
    public String getTablesSql(DatasourceRequest datasourceRequest) throws Exception {
        OscarConfig oscarConfig = new Gson().fromJson(datasourceRequest.getDatasource().getConfiguration(), OscarConfig.class);
        if (StringUtils.isEmpty(oscarConfig.getSchema())) {
            throw new Exception("Database schema is empty.");
        }
        //return "select table_name from all_tab_comments where owner='OWNER' AND table_type = 'TABLE' ".replaceAll("OWNER", oscarConfig.getSchema());
        return ("select table_name from all_tables where owner=upper('OWNER') ").replaceAll("OWNER",
                oscarConfig.getSchema());
    }

    @Override
    public String getSchemaSql(DatasourceRequest datasourceRequest) {
        return "select * from all_users";  //"select OBJECT_NAME from dba_objects where object_type='SCH'";
    }
