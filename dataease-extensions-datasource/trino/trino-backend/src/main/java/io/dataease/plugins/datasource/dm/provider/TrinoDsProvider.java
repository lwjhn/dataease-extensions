...
public class TrinoDsProvider extends DefaultJdbcProvider {
    ...
    public Connection getConnection(DatasourceRequest datasourceRequest) throws Exception {
        TrinoConfig trinoConfig = new Gson().fromJson(datasourceRequest.getDatasource().getConfiguration(), TrinoConfig.class);
        ...
        if(StringUtils.isNotEmpty(trinoConfig.getAuthMethod()) && trinoConfig.getAuthMethod().equalsIgnoreCase("kerberos")){
            ...
            loginUserFromKeytab.invoke(null, trinoConfig.getUsername(), "/opt/dataease/conf/" + trinoConfig.getPassword());
        }else {
            if (StringUtils.isNotBlank(trinoConfig.getUsername())) {
